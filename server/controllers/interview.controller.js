...
import { askAi } from "../services/openRouter_service.js";
...
`
    ...
    const interview = await Interview.create({
      ...
      questions: questionsArray.map((q, index) => ({
        ...
        timeLimit: [60, 60, 99, 90, 120][index],
      }))
    })
    res.json({
...
export const submitAnswer = async (req, res) => {
  try {
    ...
    // ✅ FIX 1: Return score: 0 in the no-answer response so frontend can update correctly
    if (!answer) {
      question.score = 0;
      ...
      return res.json({
        ...
        score: 0  // ✅ ADDED — was missing before, causing frontend score state to not update
      });
    }
...
`
    ...
    // ✅ FIX 2: Strip markdown fences before parsing (AI sometimes wraps JSON in ```json ... ```)
    ...
    return res.status(200).json({
      ...
      score: parsed.finalScore  // ✅ already present — kept as-is
    });
